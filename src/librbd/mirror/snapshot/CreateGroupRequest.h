...
#ifndef CEPH_LIBRBD_MIRROR_SNAPSHOT_CREATE_GROUP_REQUEST_H
#define CEPH_LIBRBD_MIRROR_SNAPSHOT_CREATE_GROUP_REQUEST_H

#include "include/buffer.h"
#include "include/rados/librados.hpp"
//#include "librbd/ImageCtx.h"
// following needed for error: invalid use of incomplete type ‘class librbd::ImageWatcher<librbd::ImageCtx>’
...
public:
  static CreateGroupRequest *create(librados::IoCtx& group_ioctx,
                                    const std::string& group_name,
                                    uint32_t flags, std::string *snap_id,
                                    Context *on_finish) {
    return new CreateGroupRequest(group_ioctx, group_name, flags, snap_id,
                                  on_finish);
  }

  CreateGroupRequest(librados::IoCtx& group_ioctx,
                     const std::string& group_name,
                     uint32_t flags, std::string *snap_id,
                     Context *on_finish)
...
extern template class librbd::mirror::snapshot::CreateGroupRequest<librbd::ImageCtx>;

#endif // CEPH_LIBRBD_MIRROR_SNAPSHOT_CREATE_GROUP_REQUEST_H
