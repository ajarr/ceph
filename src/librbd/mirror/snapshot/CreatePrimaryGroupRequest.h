...
#ifndef CEPH_LIBRBD_MIRROR_SNAPSHOT_CREATE_PRIMARY_GROUP_REQUEST_H
#define CEPH_LIBRBD_MIRROR_SNAPSHOT_CREATE_PRIMARY_GROUP_REQUEST_H
...
class CreatePrimaryGroupRequest {
public:
  static CreatePrimaryGroupRequest *create(librados::IoCtx& group_ioctx,
                                    const std::string& group_name,
                                    uint32_t flags, std::string *snap_id,
                                    Context *on_finish) {
    return new CreatePrimaryGroupRequest(group_ioctx, group_name, flags,
                                         snap_id, on_finish);
  }

  CreatePrimaryGroupRequest(librados::IoCtx& group_ioctx,
                     const std::string& group_name,
                     uint32_t flags, std::string *snap_id,
                     Context *on_finish)
...
private:
   ...
   *    |
   *    |                 (on error)
   * CREATE IMAGE SNAPS . . . . . . . REMOVE INTERIM SNAPSHOTS
   *    |                                  .
   *    v                                  .
   * NOTIFY UNQUIESCE < . . . . . . . . .  .
   *    |
  ...
  // cleanup
  void remove_interim_snapshots();
  void handle_remove_interim_snapshots(int r);
};
...
extern template class librbd::mirror::snapshot::CreatePrimaryGroupRequest<librbd::ImageCtx>;

#endif // CEPH_LIBRBD_MIRROR_SNAPSHOT_CREATE_PRIMARY_GROUP_REQUEST_H
