...
void GroupReplayer<I>::bootstrap_group() {
  ...
  set_mirror_group_status_update(
    true, cls::rbd::MIRROR_GROUP_STATUS_STATE_STARTING_REPLAY, "bootstrapping");
  request->send();
}
...
void GroupReplayer<I>::handle_create_group_replayer(int r) {
  ...
  set_mirror_group_status_update(
     true, cls::rbd::MIRROR_GROUP_STATUS_STATE_REPLAYING, "replaying");
...
void GroupReplayer<I>::start_image_replayers() {
  ...
  set_mirror_group_status_update(
    true, cls::rbd::MIRROR_GROUP_STATUS_STATE_STARTING_REPLAY, "starting replay");
...
void GroupReplayer<I>::shut_down(int r) {
  ...
  Context *ctx = new LambdaContext(
    [this, r](int _r) {
      set_mirror_group_status_update(true, m_status_state, m_state_desc);
      handle_shut_down(r);
    });
...
void GroupReplayer<I>::update_mirror_group_status() {
  ...
  auto ctx = new LambdaContext(
    [this](int r) {
      set_mirror_group_status_update(false, boost::none, "");
      m_in_flight_op_tracker.finish_op();
    });
...
void GroupReplayer<I>::set_mirror_group_status_update(
    bool force, const OptionalMirrorGroupStatusState &opt_status_state,
    const std::string &desc) {
  dout(20) << "force=" << force << "state=" << opt_status_state
           << ", description=" << desc << dendl;
  ...
  if (m_remote_group_peer.mirror_status_updater != nullptr) {
    m_remote_group_peer.mirror_status_updater->set_mirror_group_status(
        m_global_group_id, remote_status, force);
  }
}
